...
class E2e(unittest.TestCase):
    ...
    def test(self):
        ...
        test_team_data = (
            our_db.session.query(TeamData).order_by(TeamData.match_id.desc()).all()
        )
        ...
        for i, _ in enumerate(prod_team_data):
            ...
            self.assertEqual(
                str(prod_team_data[i]["game_version"]),
                str(test_team_data[i].game_version),
            )
            self.assertEqual(prod_team_data[i]["win"], str(test_team_data[i].win))
            self.assertEqual(
                prod_team_data[i]["participants"], str(test_team_data[i].participants)
            )
            self.assertEqual(
                prod_team_data[i]["first_blood"], str(test_team_data[i].first_blood)
            )
            self.assertEqual(
                prod_team_data[i]["first_baron"], str(test_team_data[i].first_baron)
            )
            self.assertEqual(
                prod_team_data[i]["first_tower"], str(test_team_data[i].first_tower)
            )
            self.assertEqual(
                prod_team_data[i]["first_dragon"], str(test_team_data[i].first_dragon)
            )
            self.assertEqual(
                str(prod_team_data[i]["first_inhib"]),
                str(test_team_data[i].first_inhib),
            )
            self.assertEqual(
                str(prod_team_data[i]["first_rift_herald"]),
                str(test_team_data[i].first_rift_herald),
            )
            self.assertEqual(
                str(prod_team_data[i]["ally_dragon_kills"]),
                str(test_team_data[i].ally_dragon_kills),
            )
            self.assertEqual(
                str(prod_team_data[i]["ally_rift_herald_kills"]),
                str(test_team_data[i].ally_rift_herald_kills),
            )

            self.assertEqual(
                str(prod_team_data[i]["enemy_dragon_kills"]),
                str(test_team_data[i].enemy_dragon_kills),
            )
            self.assertEqual(
                str(prod_team_data[i]["enemy_rift_herald_kills"]),
                str(test_team_data[i].enemy_rift_herald_kills),
            )

            self.assertEqual(
                str(prod_team_data[i]["inhib_kills"]),
                str(test_team_data[i].inhib_kills),
            )
            self.assertEqual(
                str(prod_team_data[i]["bans"]), str(test_team_data[i].bans)
            )
            self.assertEqual(
                str(prod_team_data[i]["enemy_bans"]), str(test_team_data[i].enemy_bans)
            )
            self.assertEqual(
                str(prod_team_data[i]["allies"]), str(test_team_data[i].allies)
            )
            self.assertEqual(
                str(prod_team_data[i]["enemies"]), str(test_team_data[i].enemies)
            )
            self.assertEqual(
                str(prod_team_data[i]["start_time"]), str(test_team_data[i].start_time)
            )
        ...
        for user in users:
            ...
            prod_user_data = json.loads(
                requests.get(f"{dns}/api/get_user_data?name={user}", timeout=200).text
            )

            test_user_data = (
                our_db.session.query(MatchData)
                .filter_by(player=user)
                .order_by(MatchData.match_id.desc())
                .all()
            )
            ...
            for i, _ in enumerate(prod_user_data):
                self.assertEqual(
                    str(prod_user_data[i]["match_id"]), str(test_user_data[i].match_id)
                )
                self.assertEqual(
                    str(prod_user_data[i]["player"]), str(test_user_data[i].player)
                )
                self.assertEqual(
                    str(prod_user_data[i]["role"]), str(test_user_data[i].role)
                )
                self.assertEqual(
                    str(prod_user_data[i]["champion"]), str(test_user_data[i].champion)
                )
                self.assertEqual(
                    str(prod_user_data[i]["champion_name"]),
                    str(test_user_data[i].champion_name),
                )
                self.assertEqual(
                    str(prod_user_data[i]["enemy_champion"]),
                    str(test_user_data[i].enemy_champion),
                )

                self.assertEqual(
                    str(prod_user_data[i]["enemy_champion_name"]),
                    str(test_user_data[i].enemy_champion_name),
                )

                self.assertEqual(
                    str(prod_user_data[i]["first_blood"]),
                    str(test_user_data[i].first_blood),
                )
                self.assertEqual(
                    str(prod_user_data[i]["first_blood_assist"]),
                    str(test_user_data[i].first_blood_assist),
                )
                self.assertEqual(
                    str(prod_user_data[i]["kills"]), str(test_user_data[i].kills)
                )
                self.assertEqual(
                    str(prod_user_data[i]["assists"]), str(test_user_data[i].assists)
                )
                self.assertEqual(
                    str(prod_user_data[i]["deaths"]), str(test_user_data[i].deaths)
                )
                self.assertEqual(
                    str(prod_user_data[i]["damage_to_champs"]),
                    str(test_user_data[i].damage_to_champs),
                )
                self.assertEqual(
                    str(prod_user_data[i]["damage_to_turrets"]),
                ...
                if test_user_data[i].gold_per_minute:
                    self.assertEqual(
                        prod_user_data[i]["gold_per_minute"],
                        f"{float(test_user_data[i].gold_per_minute):g}",
                    )

                if test_user_data[i].creeps_per_minute:
                    self.assertEqual(
                        prod_user_data[i]["creeps_per_minute"],
                        f"{float(test_user_data[i].creeps_per_minute):g}",
                    )

                if test_user_data[i].xp_per_minute:
                    self.assertEqual(
                        prod_user_data[i]["xp_per_minute"],
                        f"{float(test_user_data[i].xp_per_minute):g}",
                    )

                self.assertEqual(
                    str(prod_user_data[i]["wards_placed"]),
                    str(test_user_data[i].wards_placed),
                )

                self.assertEqual(
                    str(prod_user_data[i]["vision_wards_bought"]),
                    str(test_user_data[i].vision_wards_bought),
                )

                self.assertEqual(
                    str(prod_user_data[i]["wards_killed"]),
                    str(test_user_data[i].wards_killed),
                )
                self.assertEqual(
                    str(prod_user_data[i]["items"]), str(test_user_data[i].items)
                )
                self.assertEqual(
                    str(prod_user_data[i]["perks"]), str(test_user_data[i].perks)
                )

        # print("Checking json_data")

        print("Checking Script Runs")
        my_runs_data = requests.get(f"{dns}/api/get_script_runs", timeout=200)
        ...
        for i, _ in enumerate(prod_run_data):
            ...
            self.assertEqual(
                prod_run_data[i]["matches_added"], test_run_data[i].matches_added
            )

...
def main():
    ...
    team_data = requests.get(
        "http://paulzplace.asuscomm.com/api/get_team_data?name=" + sys.argv[1],
        timeout=200,
    )
    ...
    if maxgames == "All":
        ...
        merged_recent = merged_matches.sort_values("start_time", ascending=False).head(
            int(maxgames)
        )
...
def get_player_data(player: str) -> pd.DataFrame:
    ...
    return pd.DataFrame(
        json.loads(
            requests.get(
                f"http://paulzplace.asuscomm.com/api/get_user_data?name={player}",
                timeout=200,
            ).text
        )
    )
...
def get_performance_for_champ_role(
    player_matches: pd.DataFrame,
    what_role=None,  # pylint: disable-msg=too-many-locals
    incnone="false",
) -> pd.DataFrame:
    ...
    for champ in champ_list:
        ...
        for role in role_list:
            # win ratio first
            champ_games_role = player_champ_matches[
                player_champ_matches["role"] == role
            ]
            ...
            if champ_wins > 0 or champ_fails > 0:
                ...
                cspm = (
                    pd.to_numeric(champ_games_role["creeps_per_minute"]).mean().round(2)
                )
                tower_dmg = (
                    pd.to_numeric(champ_games_role["damage_to_turrets"]).mean().round(2)
                )
                champ_dmg = (
                    pd.to_numeric(champ_games_role["damage_to_champs"]).mean().round(2)
                )
                wards_placed = (
                    pd.to_numeric(champ_games_role["wards_placed"]).mean().round(2)
                )

                vision_bought = (
                    pd.to_numeric(champ_games_role["vision_wards_bought"])
                    .mean()
                    .round(2)
                )

                wards_killed = (
                    pd.to_numeric(champ_games_role["wards_killed"]).mean().round(2)
                )
                ...
                if pd.to_numeric(champ_games_role["deaths"]).mean() != 0:
                    ...
                    kda = (
                        (
                            pd.to_numeric(champ_games_role["kills"]).mean()
                            + pd.to_numeric(champ_games_role["assists"]).mean()
                        )
                        / pd.to_numeric(champ_games_role["deaths"]).mean()
                    ).round(2)
                else:

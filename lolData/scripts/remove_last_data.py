...
class RemoveDB(unittest.TestCase):
    ...
    def test_after_removing(self):
        ...
        our_mongo = LolMongo(
            config.mongo_host, config.mongo_user, config.mongo_pw, config.mongo_name
        )
        ...
        existing_match_history = our_db.session.query(TeamData).order_by(
            TeamData.match_id.desc()
        )
        matches_to_remove = existing_match_history[:20]

        print(
            f"Removing these matches: {[match.match_id for match in matches_to_remove]}"
        )
        ...
        for match in matches_to_remove:
            for particip in match.participants.split(","):
                ...
                our_db.session.query(MatchData).filter_by(
                    match_id=match.match_id
                ).filter_by(player=particip.strip()).delete()
        ...
        self.assertEqual(
            len(pre_match_data) - num_matches_removed, len(current_match_data)
        )

...
def main():
    ...
    # script runs table.
    my_script_run_data = requests.get(f"{dns}/api/get_script_runs", timeout=200)
    ...
    # league_users table
    my_league_user_data = requests.get(f"{dns}/api/get_league_users", timeout=200)
...
def get_player_data(player: str) -> list:
    ...
    """
    return json.loads(
        requests.get(f"{dns}/api/get_user_data?name={player}", timeout=200).text
    )
